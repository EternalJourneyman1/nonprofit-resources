...
const formatDate = (user: User | null, dateType: 'creationTime' | 'lastSignInTime'): string => {
    ...
    <div class="bg-white p-8 rounded-lg shadow-md">
      ...
      <div v-if="auth.user" class="space-y-4">
        <div class="flex items-center space-x-4">
          ...
          >
          <div>
            <h2 class="text-xl font-semibold">{{ auth.user.displayName || 'Anonymous User' }}</h2>
        ...
        <div class="mt-8">
          ...
          <p class="text-gray-600">Member since: {{ formatDate(auth.user, 'creationTime') }}</p>
          <p class="text-gray-600">Last login: {{ formatDate(auth.user, 'lastSignInTime') }}</p>
        </div>
      </div>
